...
def application_list(request):
    ...
    if role == "master":
        ...
        if status_id:
            ...
            applications = applications.filter(status_id=1)
...
def fastapplication_list(request):
    ...
    if request.method == "POST":
        ...
        selected_packages = list(
            map(int, request.POST.getlist("selected_package_devices", [])))
...
def yagpt_page(request):
    if request.method == "GET":
        ...
        if role == "master":
            ...
            notifications_count = applications.count()
        else:
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        {"role": "system",
                         "text": "Ты — опытный компьютерный мастер. Помогай пользователям с компьютерными проблемами, настройками, сборкой ПК и программным обеспечением, ответ вмести в 850 симолов."},
